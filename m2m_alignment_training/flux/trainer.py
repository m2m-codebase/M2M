...
class Trainer:
    ...
    def train(self, start_epoch, end_epoch):
        ...
        for epoch in range(start_epoch, end_epoch+1):
            ...
            for idx, batch in tqdm(enumerate(self.train_loader), desc=f"Epoch {epoch}", total=len(self.train_loader)):
                ...
                for key, value in batch.items():
                    if isinstance(value, torch.Tensor):
                        batch[key] = value.to(self.device).to(dtype=torch.bfloat16)
            ...
            if epoch%10 == 0:
                save_path = os.path.join(self.args.save_dir, self.args.wandb_name, f"epoch_{epoch}.pth")
    ...
    @torch.no_grad()
    def evaluate(self, return_top_idx=False, use_clip=False, labse_baseline=False):
        print('Evaluating...')
        self.model.eval()
        ...
        for lang in self.eval_dataset.lang_cols:
            ...
            i2t_recall = {f'eval/R@{k}/i2t/{lang}': (i2t_is_correct_in_top_k[:, :k].sum(dim=1) > 0).float().mean().item()*100 for k in self.k_values}
            t2i_recall = {f'eval/R@{k}/t2i/{lang}': (t2i_is_correct_in_top_k[:, :k].sum(dim=1) > 0).float().mean().item()*100 for k in self.k_values}
        ...
        metrics.update({
            "eval/avg_metric_t2i": avg_metric_t2i,
            "eval/avg_metric_i2t": avg_metric_i2t,
            "eval/avg_metric": avg_metric,
        })
        wandb.log(metrics)
