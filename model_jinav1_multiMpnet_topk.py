...
class labse_clip(torch.nn.Module):
    ...
    def kl_clip_loss(self, outputs, use_mse=False, use_l1=False):
        ...
        # B x K
        gt_topk_sim, gt_topk_idx = gt_sim_score.topk(k=self.args.topk, dim=-1)
        ...
        if use_mse:
            kl_loss = torch.nn.functional.mse_loss(pred_topk_sim, gt_topk_sim, reduction='mean')
        elif use_l1:
            kl_loss = torch.nn.functional.l1_loss(pred_topk_sim, gt_topk_sim, reduction='mean')
