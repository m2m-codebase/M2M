...
class labse_clip(torch.nn.Module):
    ...
    def kl_clip_loss(self, outputs, use_mse=False, use_l1=False):
        ...
        gt_sim_score.fill_diagonal_(MIN)
        gt_topk_sim, gt_topk_idx = gt_sim_score.topk(k=self.args.topk, dim=-1) # shape BxK
        
        gt_sim_score.fill_diagonal_(-MIN)
        gt_lastk_sim, gt_lastk_idx = (-gt_sim_score).topk(k=self.args.topk, dim=-1)
        gt_lastk_sim = -gt_lastk_sim

        B = gt_sim_score.shape[0]
        assert gt_topk_sim.shape == (B, self.args.topk), f"topk shape mismatch: {gt_topk_sim.shape}; expected {(B, self.topk)}"

        row_indices = torch.arange(B).unsqueeze(1).expand(-1, self.args.topk).to(gt_sim_score.device)  # shape (B, k)
        pred_topk_sim = pred_sim_score[row_indices, gt_topk_idx]
        pred_lastk_sim = pred_sim_score[row_indices, gt_lastk_idx]

        # B x (2K)
        pred_score = torch.cat([pred_topk_sim, pred_lastk_sim], dim=-1)
        gt_score = torch.cat([gt_topk_sim, gt_lastk_sim], dim=-1)
        
        assert pred_topk_sim.shape == (B, self.args.topk), f"topk shape mismatch: {pred_topk_sim.shape}; expected {(B, self.topk)}"
        
        if use_mse:
            kl_loss = torch.nn.functional.mse_loss(pred_score, gt_score, reduction='mean')
        elif use_l1:
            kl_loss = torch.nn.functional.l1_loss(pred_score, gt_score, reduction='mean')
